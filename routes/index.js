...
router.get("/", (req, res) => {
    ...
    if (req.query.search) {
        ...
        var message = null;
        Post.find({ name: regex }).skip((perPage * pageNumber) - perPage).limit(perPage).sort({ date: "descending" }).exec((err, allPosts) => {
            Post.count({ name: regex }).exec((err, count) => {
                if (err) {
                    console.log(err);
                    res.redirect("back");
                } else {
        ...
        Post.find({}).skip((perPage * pageNumber) - perPage).limit(perPage).sort({ date: "descending" }).exec((err, allPosts) => {
